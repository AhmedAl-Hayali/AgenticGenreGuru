...
│       ├── dto.py           # transport DTOs (Artist, Album, RawDeezerTrack, DeezerSearchResponse, Track, DeezerErrorEnvelope, ...)
...
│   └── factories.py         # SongFactory, SongArtistFactory, SongFingerprintFactory (task T013)
...
| **Deezer**        | `genreguru/deezer/client.py`          | `GET /search?q=..&limit=5`, `GET /track/{id}` (contributors normalization), Track field mapping, ISRC/preview fail-loud, error-code mapping (QUOTA 4, SERVICE_BUSY 700 → retry; DATA_NOT_FOUND 800 → empty)                                                 | T022       |
...
| `fingerprint_app/ts/preview-player.ts` (+ `render.ts`, `page-controller.ts`) | Candidate preview playback: `.candidate-playback` row with round scrubber → square (durations) state morph, autoplay on select, progress loop via `requestAnimationFrame`, `previewUnavailable`/`previewFailed` surfacing; preview card prerendered by `render.ts` (cover lazy-loads via `lazy-image.ts`) | T029-030 companion             |
| `fingerprint_app/ts/lazy-image.ts`, `scroll-reveal.ts`                       | Progressive image loading + scroll-reveal animation (vanilla TS, no deps)                                                                                                                                                                                                                                 | T029-030 companion             |
...
sequenceDiagram
    ...
    DZ-->>SVC: top-5 Track[]
    SVC->>DZ: GET /track/{id} per match (best-effort enrichment)
    DZ-->>SVC: artist list + cover per match
    SVC-->>UI: top-5 enriched matches (status: success)
    UI-->>U: render candidate match selections
    U->>UI: Select & confirm candidate
...
| Artist enrichment fails       | `GenreguruError` per-track → INFO skip, best-effort; candidate match falls back to search fields | SC-enrichment                            |
...
- `GET https://api.deezer.com/track/{id}` for artist/cover enrichment (contributors normalization; `md5_image` → `cover` URL). HTTP 404 or `DATA_NOT_FOUND`(800) → `TrackNotFoundError`; retryable codes behave like search.
...
- Preview-UX coverage: `web/tests/preview-player.test.ts` (playback state machine, `.candidate-preview-row` morph, `previewUnavailable`/`previewFailed` fallbacks), `lazy-image.test.ts`, `scroll-reveal.test.ts`, and `a11y.test.ts` (roles/labels on the search + confirm flow) sit alongside the `render`/`api`/`page-controller`/`bootstrap` suites behind the ≥95% v8 gate.
...
| POST /api/confirm/ returns song_id + deezer_id + isrc + fingerprint | Contracted response shape; caller not told reuse vs fresh (logging only)              | Leaking `reused` flag to caller (rejected by contract)                               |
