...
| Django settings          | `web/genreguru_web/settings/`    | `django`, `db`           | `DEBUG`/hosts/secrets/secure flags from `django` group; `DATABASES` built from individual db components shared with core `genreguru/db/engine.py`                 |
