...
│   ├── vitest.config.ts     # Vitest + v8 coverage (≥75% gate, HTML + LCOV)
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| `genreguru_web/`                                                                                                                                       | Project settings (Hydra compose, pdoc docformat), root URL router, WSGI/ASGI entrypoints, middleware                                                                                                                                                                                                          | T003                             |
| `fingerprint_app/` views                                                                                                                               | `GET /api/search/`, `POST /api/confirm/` (implemented; match carried in request body, no id in path); `GET /api/songs/`, `GET /api/songs/{isrc}/`, feature-gated visualization + recommend endpoints (planned, US2-4)                                                                                         | T026-027, T034-035, T040, T045   |
| `fingerprint_app/urls.py` + `genreguru_web/urls.py`                                                                                                    | Route registration                                                                                                                                                                                                                                                                                            | T028, T037, T042, T047           |
...
| JS toolchain (dev)                                  | esbuild (TS→ESM bundle), ESLint 10 flat config, Prettier 3, `tsc` strict typecheck, Vitest 5 + jsdom contract tests (`frontend/tests/`), v8 coverage ≥75% gate (`npm run build`, `npm run check`, `npm run test:coverage`) | companion to T029-030         |
