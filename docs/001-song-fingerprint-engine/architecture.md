...
| **Deezer**        | `genreguru/deezer/client.py`          | `GET /search?q=..&limit=5`, `GET /track/{id}` + `enrich_artists` (contributors normalization), Track field mapping, ISRC/preview fail-loud, error-code mapping (QUOTA 4, SERVICE_BUSY 700 → retry; DATA_NOT_FOUND 800 → empty)                              | T022       |
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| `genreguru_web/`                                                                                                                                       | Project settings (Hydra compose, pdoc docformat), root URL router, WSGI/ASGI entrypoints, middleware                                                                                                                                                                                                          | T003                             |
| `fingerprint_app/` views                                                                                                                               | `GET /api/search/` (implemented with best-effort artist/cover enrichment), `POST /api/confirm/` (implemented; match carried in request body, no id in path); `GET /api/songs/`, `GET /api/songs/{isrc}/`, feature-gated visualization + recommend endpoints (planned, US2-4)                                  | T026-027, T034-035, T040, T045   |
| `fingerprint_app/urls.py` + `genreguru_web/urls.py`                                                                                                    | Route registration                                                                                                                                                                                                                                                                                            | T028, T037, T042, T047           |
