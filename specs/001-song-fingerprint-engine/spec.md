...
- PostgreSQL 18 or newer is required: all `UUID` columns use the native `uuidv7()` function as the column default, with no application-layer fallback (see data-model.md).
- Downstream recommendation algorithms will be implemented in subsequent project phases using the stored fingerprint data.

## Traceability

Original functional requirements (FR-001 → FR-016) were converted to EARS-conformant requirements (REQ-001 → REQ-019) on 2026-08-11; renumbered 2026-08-13 (REQ-002 inserted; subsequent +1; REQ-018/REQ-019 unchanged). The full mapping table, notes, and assumptions live in the source of truth [requirements.md](../../docs/001-song-fingerprint-engine/requirements.md), which is authoritative; this spec does not duplicate it.
